...
#Importing Python tools and systems that allow for the GUI and random elements of the code
import random
from tkinter import *
...
#Function that calculates the user's final grade and displays the quiz's final score screen
def calcGrade():
    ...
    playAgainFont = Font(family = "Gill Sans MT",weight="bold",size=16)
    ...
    exitButton = Button(groot, text="EXIT", font=pageFont, bg="#fd5050", fg="#FFFFFF", command=exit, borderwidth=1, padx=40, pady=10)
    exitButton.place(x=318, y=460)

    playAgain = Button(groot, text="PLAY AGAIN", font=playAgainFont, bg="#fd5050", fg="#FFFFFF", borderwidth=1, padx=4, pady=13, command=restart)
    playAgain.place(x=20, y=460)

#Function for the "Play Again" button which restarts the program
def restart():
    groot.destroy()
    initialising()

#Function which creates each question page
def answerPhase():
    root.destroy()
...
#Function for the "Skip" button which skips the question
def skipQuestion():
    global skipCounter
    skipCounter += 1
    global totalQuestions
...
#Function that checks if the answer chosen is correct and increments the user's score accordingly
...
#Funciton that creates the first title screen
def titleScreen():
    global root
    root = Tk()
    ...
    logoFile = PhotoImage(file="icon.png")
    logoInput = Label(root, image=logoFile, bg="#ffd3a6")
    logoInput.place(x=330,y=300, anchor="center")

    playButton = Button(root, text="PLAY", font=pageFont, bg="#fd5050", fg="#FFFFFF", command=answerPhase, borderwidth=1, padx=40, pady=10)
    playButton.place(x=95, y=410)
    exitButton = Button(root, text="EXIT", font=pageFont, bg="#fd5050", fg="#FFFFFF", command=exit, borderwidth=1, padx=40, pady=10)
    exitButton.place(x=395, y=410)


    root.mainloop()

#Calls the initialising function which starts up the program
initialising()